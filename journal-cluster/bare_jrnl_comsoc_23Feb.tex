...
When a CR node $i$ is associated to multiple clusters, $D_i$ is still set to zero. %Note here that connectivity degree on cluster head will still be the actual value.% The value will expire after some duration and is changed later on back to the original value. 
...
\begin{figure}[ht!]
  \centering
  \includegraphics[width=0.8\linewidth]{percentage_overlapping_node.pdf}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.6\linewidth]{final_clustering_ross.pdf}
...
%width=0.5\linewidth
\subfigure[Resulted from SOC]{\includegraphics[width=0.55\linewidth]{final_clustering_soc}\label{fig:final_clustering_soc}}
\subfigure[Resulted from Centralized Optimization]{\includegraphics[width=0.55\linewidth]{final_clustering_LP}\label{fig:final_clustering_LP}}
\end{center}
...
In order to highlight the amount of control signalling only for clustering, we omit the process of neighbourhood discovery, which is premise for all clustering schemes.%, and only compare the number of control messages brought in by the features of the schemes. 
%In different procedures of the clustering scheme, control messages are sent with either broadcast or unicast.
...
%ROSS-5-DGA achieves homogeneous distribution of cluster sizes, \ie, there is no cluster whose size is greater than 3, and the number of CRs in 2 node cluster is greater than that resulted from ROSS-2-DFA.
...
%\includegraphics[width=\columnwidth]{cdf_clusterSize_100.pdf}%
%\caption{Desired cluster size $x$ is 5. 100 CRs, 30 PRs in network, the average number of neighbours of CR node is 9.5}%
%\label{cdf_clusterSize_100}%
%\end{subfigure}\hfill%
%\begin{subfigure}{.4\columnwidth}
%\includegraphics[width=\columnwidth]{cdf_clusterSize_200.pdf}%
%\caption{Desired cluster size $x$ is 7. 200 CRs, 30 PRs in network, the average number of neighbours of CR node is 20}%
%\label{cdf_clusterSize_200}%
%\end{subfigure}\hfill%
%\begin{subfigure}{.4\columnwidth}
%\includegraphics[width=\columnwidth]{cdf_clusterSize_300.pdf}%
%\caption{cdf_clusterSize_300.pdf}\par\caption{Desired cluster size $x$ is 12. 300 CRs, 30 PRs, the average number of neighbours is 30}%
...
\bibliographystyle{IEEEtran}
\bibliography{../../thesis/thesisTemplate/backmatter/myrefs}
