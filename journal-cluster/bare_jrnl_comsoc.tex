...
After deciding itself being cluster head, CR node broadcasts to notify its neighbours on control channel, meanwhile, $i$'s initial cluster is formed immediately, which is $i$'s neighborhood except for those nodes which have become cluster heads, \ie $C_i=(\texttt{Nb}_i\setminus CHs)\cup i$.
...
\hline
 ...
 ROSS-DGA, ROSS-x-DGA 	&$\mathcal{O}(n^3)$	&   $h+2*m^2c$ (upper bound)
				& $\texttt{ID}_{H_C}$ and $V_C$ for $h+m^2c$ times, notification to join in one cluster for $m^2c$ times					\\ \hline
 ROSS-DFA, ROSS-x-DFA 	&$\mathcal{O}(n)$	&   $h+ 2m$	 (upper bound) 						& $\texttt{ID}_{H_C}$ and $V_C$ for $h+m$ times, notification to join in one cluster for $m$ times	 					\\ \hline
 SOC 					&$\mathcal{O}(n)$	&   $3*n$					& $\{V_i\}, i\in M\subseteq \texttt{Nb}_i$						\\ \hline
...
\caption{Cluster head determination and initial cluster formation for Unclustered CR node $i$}          % give the algorithm a caption
\label{alg0} 
\DontPrintSemicolon
\SetAlgoLined
...
\section*{}
\begin{algorithm}               % enter the algorithm environment
\caption{Guarantee the availability of CCC / cluster size control by cluster head}          % give the algorithm a caption
