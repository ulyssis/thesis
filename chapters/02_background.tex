...
how to design distributed algorithms on the basis of a optimization problem?

As is mentioned in Section~\ref{operation_model}, constrained optimization has been used to formulate and solve a variety of problems in wireless networks and CRN.
...
The decomposition methods are originally proposed to solve very large optimization problems, but its side effect, yielding decentralized solution algorithms is very important in many scenarios in wireless network~\cite{boyd2007notes}. 
...
Specifically, the decomposition techniques includes primal decomposition and dual decomposition.
The former decomposing the original problem by allocating the available resources to each subproblems, and the latter is based on decomposing the Lagrangian dual problem~\cite{Bertsekas_99, Palomar06atutorial}.

We take the joint channel and power allocation problem in CRN, which is addressed in~\cite{tachwali_opt_bandwidth_power_2013} as an example, in order to show how is one distributed algorithms is obtained by decomposing the optimization problem.
To eliminate the impact of the coupling optimization variable, \ie channel and power levels, primal decomposition derives two subproblems, channel allocation and power control. 
As to each subproblem, Lagrangian method is used to eliminate the coupling constraints.
The sequential solution of the two subproblems form one iteration, and the iterations cease when the change of the objective is below a given stopping criterion.
...
\hline
Physical layer & transmission power, modulation, channel coding rate, spectrum bandwidth & minimal over power consumption, maximal throughput, minimal BER, maximize product of bandwidth of power~\cite{tachwali_opt_bandwidth_power_2013} & maximal transmission power, caused interference on licensed users, available channel coding rate \\ 
\hline
\end{tabular} 
