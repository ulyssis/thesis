...
CRAHN is composed with autonomous mobile cognitive radio users which work with overlay spectrum sharing paradigm.
...
As to the various challenging problems in CRN networks, adopting centralized or decentralized solutions is largely dependant on the network types and the nature of the problem to be solved.
...
CRAHNs consist of mobile autonomous secondary users and don't have infrastructure, thus the network topology evolves with time, besides, the ungovernable primary users' activity leads to the temporal spectrum availability. 
These factors make distributed scheme to be the sole choice to manage the CRAHN to convey services.
%
Traditional commercial cellular networks work with licensed spectrum, which operate with centralized manner to provide services meeting QoS criterion.
The openness of TV white space attracts private people and organizations to build cellular networks to launch various services.
...
But owners of the cellular networks have different or conflicting interests, thus it may not be possible to let every secondary user to follow the strategy given by the centralized data base~\cite{crn_futurecellular_2014}.
...
When designing distributed solution is necessary, which 
...
If the algorithm converges, in which state the algorithm finally ceases, and how robust and quickly the convergence process is?
...
\subsection{Utilization of TV white spectrum}
...
The benefits promised by cluster structure is challenged by the ungovernable activity of primary users.
...
Based on the description of the challenges, we formulate the 
three research questions which will be answered in this thesis.
...
xxxx
In this dissertation, we meet the challenges in CRN by proposing distributed solutions.
...
\subsection*{Contribution 1: Utilization of TV white spectrum and application of game model in designing algorithm}
...
\subsection*{Contribution 2: Versatile robust clustering in CRAHN}
...
\subsection*{Contribution 3: Spectrum aware virtual coordinates and geographic routing in CRN}
