...
The first one depicts the functionalities of cognitive radio, and the second one describe the role of cognitive radio in a network.
...
How is the available licensed spectrum shared among secondary users is a fundamental problem in CRN. Spectrum sharing decides secondary users' performance, and it also poses threat of interfering primary users.
...
Working with this paradigm, secondary users should monitor the spectrum of interest preactively to detect primary users' appearance.
...
\subsection{Centralized and Distributed Solutions}
Based on management architecture, spectrum sharing can be classified as centralized and distributed spectrum sharing.
...
There is a considerable number of centralized approaches proposed for spectrum sharing in cognitive radio network, and the global optimality is reported as to different objectives.
...
Nevertheless, centralized solution is not suitable in many other scenarios.
...
When primary users intensely access the spectrum and result in frequent change of available spectrum, the control overhead becomes higher, and it is different for the centralized entity to obtain a full and up to date picture of the spectrum availability in the whole network.
...
When working with distributed spectrum sharing paradigm, secondary users decide their spectrum usage strategies autonomously based on the information on themselves or from their neighbourhoods.
Consequently, distributed scheme usually requires secondary users to exchange information \ie user ID, channel availability, etc. 
with its neighbours.
It is reported that control overhead takes more than 50\% of messages~\cite{Han:2008:RAW:1457343}.
If we can reduce the overhead, spectrum utilization, the number of served users and the network performance can be correspondingly improved.
...
In certain scenario, distributed scheme may also makes use of centralized entity when it is available and necessary, \ie secondary users can access the centralized database to retrieve available channels in IEEE 802.22 cellular networks.
...
Although distributed scheme entails appealing lightweight computation and overhead, it is actually challenging to design in many scenarios.
...
\section{Distributed Solutions and Game in CRN}
