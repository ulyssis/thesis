...
Spectrum sharing contains two related sub problems, the spectrum sharing between the primary users and secondary users, and that among the secondary users.
The first sub problem is fundamental to the hierarchical access model, as it poses threat of interfering primary users.
The second sub problem decides the services people can obtain from CRN.
Thus, spectrum sharing largely defines the challenges and considerations one cognitive radio network faces.
In the following, we introduce the first sub problem of spectrum sharing, and deal with the second one when we take about the individual problems.
%In this section, we introduce spectrum sharing from three different perspectives.

There are two spectrum sharing models, spectrum underlay and overlay.
They origin from the instinctive understandings of people as to the hierarchical access model, and the different claims on how to effectively protect the primary users' interests.
...
\subsection{Operation model of CRN}
...
As distributed schemes exploit mainly local observation, distributed schemes help the secondary users to adapt to the varying environment quite well~\cite{Selforganization_CRN_13}.
...
The distributed schemes designed for CRN can be roughly grouped into the following categories.
\begin{itemize}
\item Heuristic methods. 
This is the most common method.
\item Learning schemes.
