...
This phase ends after a period of time to make sure each node obtains $t$th element in its virtual coordinate.

All the anchors sequentially broadcast the anchor messages which are flooded over the network, so that a secondary user obtains its virtual coordinate which respects all anchor nodes.
...
In the following two subsections, we introduce two normalized spectrum utilities on secondary user respectively, one of them is called \textit{normalized spectrum availability} which is on the basis of duty cycle of primary users' absence, the other is called \textit{normalized longest blocking time} which as the name tells, is based on the lengths of time durations that primary users are detected.
