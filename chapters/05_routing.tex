\chapter{Decentralized Virtual Coordinate Assignment and Routing}
% SPECTRUM AWARE VIRTUAL COORDINATE ASSIGNMENT AND ROUTING IN MULTIHOP COGNITIVE RADIO NETWORK
\begin{quote}
{\textbf{Abstract}\\
...
The number of anchor messages involved in this process is linear to the network size, which is supported by the following proofs.
\begin{lemma}
\label{savic:lemma1}
...
% \lnl{InRes1} regulate the line number
\begin{algorithm}[!h]
\caption{Assignment of $vc_i$ for secondary user $i$ with respect to one anchor}%, $\lambda_i$ is normalized spectrum availability on $i$}          % give the algorithm a caption
...
%\lnl{InRes3}$VC\_counter=\Psi_i$ \\ %\tcc*[f]{or $\Lambda_i$, based on }
 set $counter1=\lambda_i$ in anchor message;\\
 broadcast anchor message;\\
}

\If{$i$ receives anchor message}{
\eIf{$counter +\lambda_i \geqslant vc_i$}{
drop anchor message;}{
%\lnl{InRes9}$VC\_counter + \Psi_i < VC_i$  \\
$vc_i=counter + \lambda_i$;\\
set $counter=vc_i$ in anchor message;\\
broadcast anchor message;
}
}
\end{algorithm}
