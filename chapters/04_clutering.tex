...
Assume CR nodes are evenly distributed and cluster head locates in the middle of each cluster, the initial cluster formed is the neighborhood of cluster head, which is influenced by the transmission range and network density.
In extremely dense CRN, when one cluster is formed by CR node $i$, based on the rule for cluster head selection Algorithm~\ref{alg0}, the nearest cluster head generated could locate just outside the neighborhood or transmission range of $i$, which is as Figure~\ref{clusters_denseNetwork} shows.
Bigger black dots in the middle of clusters are clusters heads.
Note that no cluster head resides on the edge, which is due to Algorithm~\ref{alg0}: the node with biggest Individual connection degree becomes cluster head, then nodes on edge are unlikely to become cluster heads as their neighbourhoods are only half the nodes locating away from edge.
...
\end{figure}
Assume all the CRs are evenly distributed in the square plane, $l$ is the length of side of simulation plan square, and $r$ is CR's transmission radius.
...
Now we verify the estimation by simulation.
