...
%Hence, there has been great effort on the research on clustering of cognitive radio networks in recent years~\cite{Chen07,Lazos09}.
...
Prior to the emergence of open spectrum access, as an important method to manage network, clustering has been discussed by a huge column of works. 
\todo[inline]{expand}clustering is proposed in ad hoc network or mesh network~\cite{Abbasi_survey_07}. 
...
As to cognitive radio network, there are also a bunch of clustering schemes proposed, which target aspects.
...
The set of primary users and secondary users are presented with $\mathcal{P}$ and $\mathcal{N}$ separately, and there are $|\mathcal{P}| = P$ and $|\mathcal{N}| = N$.
The collection of non-overlapping licensed frequency bands is denoted as $\mathcal{F}$, $|\mathcal{F}| =F$.
...
Independent on specific channel, the primary and secondary users have fixed transmission ranges respectively\footnote{This assumption is made to simplify the discussion, and doesn't affect the effectiveness of the proposed scheme}.
...
Secondary users conduct spectrum sensing independently, by which the secondary users validate the channels to be available with a certain probability of detection as well as with a certain periodicity, i.e., every $T_{\mathrm{sense}}$ time the currently used channels need to be validated again.
...
Any user outside the transmission range of the transmitter can not receive data from it (i.e., any CR user outside primary users' transmission ranges can not ). 
...
In the following, we focus on the operation of cognitive radio network (CRN) which is constituted by all the secondary users in $\mathcal{N}$.
...
For secondary node $i$ in CRN, its neighborhood $Nb_i$ consists of all the secondary users locating within its transmission range (links are assumed to be reciprocal). 
...
\textit{Social Connection Degree} $G_i$: $G_i=|\bigcap_{j\in Nb_i}V_j|$, which is the number of common channels in $Nb_i$. $G_i$ represents the ability of a neighborhood to form a robust cluster. Figure~\ref{fig1} illustrates an example CRN where the corresponding \textit{Spectrum/Social Connection Degree} are specified for each node.
\begin{figure}[ht!]
  \centering
...
The pseudo code of cluster head determination algorithm in Phase I is in Algorithm \ref{alg0}.
...
\begin{figure}[ht!]
  \centering
  \includegraphics[width=0.4\linewidth]{clusters_denseNetwork.pdf}
...
\todo[inline]{check, the language is terrible}
In case of dense network, the cluster size is big, which brings heavy burden on cluster head.
...
\begin{algorithm}               % enter the algorithm environment
\caption{available CCC guarantee, or cluster size control conducted by cluster head}          % give the algorithm a caption
...
\begin{algorithm}               % enter the algorithm environment
\caption{Debatable node $i$ decides its affiliation}          % give the algorithm a caption,  cluster to settle
\label{alg4}
\DontPrintSemicolon
\SetAlgoLined
