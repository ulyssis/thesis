...
The pseudo code for deciding cluster head and forming initial clusters is in Algorithm~\ref{alg0}.
...
\Else{ 	
\tcc*[r]{$\tau_1 \neq \emptyset, \tau_1$ is $\emptyset$}\mbox{}
...
\begin{figure}[ht!]
  \centering
  \includegraphics[width=0.65\linewidth]{figure2.pdf}
...
As to singleton congestion game, there exists pure equilibria which can be reached with best response update~\cite{Ackermann06purenash}.
...
\subsubsection*{Complexity of Clustering Problem}
...
\begin{itemize}
\item \textit{Average number of common channels per un-singleton cluster.} 
This metric shows the robustness of the current un-singleton clusters.
...
This is a straight forward metric on the robustness of clusters.
...
\begin{figure}[!h]
  ...
  \caption{Cumulative distribution of CRs residing in clusters with different sizes. Desired cluster size $x$ is . 300 CRs, 30 PRs, the average number of neighbours is 30}
